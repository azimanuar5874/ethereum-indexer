...
class ProducerFactory(
    ...
    fun createInternalItemEventsWithoutMetaProducer(): RaribleKafkaProducer<NftItemEventDto> {
        return RaribleKafkaProducer(
            clientId = "$clientId.internal.item.without.meta",
            valueSerializerClass = JsonSerializer::class.java,
            valueClass = NftItemEventDto::class.java,
            defaultTopic = ItemMetaExtenderHandler.getInternalTopicWithoutMeta(environment, blockchain.value),
            bootstrapServers = kafkaReplicaSet
        )
    }
