...
class ItemMetaExtenderHandler(
    ...
    companion object {
        fun getInternalTopicWithoutMeta(environment: String, blockchain: String): String {
            return "protocol.$environment.$blockchain.indexer.nft.item.without.meta"
        }

        fun createItemMetaExtenderConsumer(
            ...
            val consumerGroup = "${environment}.protocol.${blockchain.value}.nft.indexer.item.meta.extender"
            val clientIdPrefix = "$environment.${blockchain.value}.$host.${UUID.randomUUID()}"
            return RaribleKafkaConsumer(
                clientId = "$clientIdPrefix.nft.indexer.item.meta.extender",
                valueDeserializerClass = JsonDeserializer::class.java,
                valueClass = NftItemEventDto::class.java,
                consumerGroup = consumerGroup,
                defaultTopic = getInternalTopicWithoutMeta(environment, blockchain.value),
